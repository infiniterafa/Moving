...
public class PlacementGridData
{
    ...
    public void AddCellObject(int index, int ID, Vector3Int currentTilePosition, Vector2Int objectSize, int rotation, int objectRotation)
    {
        List<Vector3Int> positionsToOccupy = GetCellPositions(currentTilePosition, objectSize, rotation);
    ...
    {
        ///On our grid we have stored duplicate references - so an object that took 4 cells
        ///would make all 4 cells point to the same object. Because of this when we save we need
        ///to remove the 3 additional reference to this object and only save one using
        ///the ORIGIN position that we have saved.
        ///The code below removes the duplicates from the save data:

        List<Vector3Int> cellPosition = new(gridCellsDictionary.Keys);
        List<Vector3Int> cellPositionToSave = new();
        List<CellObjectSaveData> cellObject = new(gridCellsDictionary.Values.Select(x => x.GetSaveData()));
        List<CellObjectSaveData> cellObjectToSave = new();
        for (int i = 0; i < cellPosition.Count; i++)
        {
            if(cellObjectToSave.Any(x => x.origin == cellObject[i].origin))
            {
                continue;
            }
            cellPositionToSave.Add(cellPosition[i]);
            cellObjectToSave.Add(cellObject[i]);
        }

        ///We do the same "remove duplicate" logic for edge objects - like in-wall objects
        ///takes usually 2 edge spaces
        EdgesData edgeData = EdgesData.GetEdgesData(gridEdgesDictionary.Keys);
        List<EdgeObjectSaveData> edgeObjectData = new(gridEdgesDictionary.Values.Select(x => x.GetSaveData()));
        List<Edge> edgesToSave = new();
        List<EdgeObjectSaveData> edgeDataToSave = new();
        for (int i = 0; i < edgeObjectData.Count; i++)
        {
            if(edgeDataToSave.Any(x => x.origin == edgeObjectData[i].origin))
            { 
                continue; 
            }
            edgesToSave.Add(new Edge(edgeData.smallerPoints[i], edgeData.biggerPoints[i]));
            edgeDataToSave.Add(edgeObjectData[i]);
        }

        //Here we construct the save data as a struct
        return new()
        {
            edgesData = EdgesData.GetEdgesData(edgesToSave),
            edgesObjectData = edgeDataToSave,
            cellsData = cellPositionToSave,
            cellObjectData = cellObjectToSave
        };
    }
}
