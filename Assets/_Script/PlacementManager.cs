...
/// Clase de alto nivel que conecta el input, el sistema de undo y el placement system
...
public class PlacementManager : MonoBehaviour
{
    ...
    private bool m_wasInMovementState = false;
    ...
    /// La unión del placement State que conecta al Input system
    ...
    /// Conecta el input al Building State
    ...
    private void HandleDeleteAction(bool val)
    {
        ...
        {
            if (m_wasInMovementState)
            {
                m_wasInMovementState = false;
                HandleMoveObject();
            }
            else
            {
    ...
    public void PlaceStructureAt(SelectionResult selectionResult, PlacementGridData placementData, ItemData itemData, bool animatePlacement = true)
    {
        for (int i = 0; i < selectionResult.selectedGridPositions.Count; i++)
        {
            if (itemData.objectPlacementType.IsEdgePlacement())
            {
                int objectIndex = structurePlacer.PlaceStructure(itemData.prefab, selectionResult.selectedPositions[i], selectionResult.selectedPositionsObjectRotation[i], 0, animatePlacement);
                placementData.AddEdgeObject(objectIndex, itemData.ID, selectionResult.selectedGridPositions[i], itemData.size, Mathf.RoundToInt(selectionResult.selectedPositionGridCheckRotation[i].eulerAngles.y), Mathf.RoundToInt(selectionResult.selectedPositionsObjectRotation[i].eulerAngles.y));
            }
            else
            {
                int objectIndex = structurePlacer.PlaceStructure(itemData.prefab, selectionResult.selectedPositions[i], selectionResult.selectedPositionsObjectRotation[i], 0, animatePlacement);
                placementData.AddCellObject(objectIndex, itemData.ID, selectionResult.selectedGridPositions[i], itemData.size, Mathf.RoundToInt(selectionResult.selectedPositionGridCheckRotation[i].eulerAngles.y), Mathf.RoundToInt(selectionResult.selectedPositionsObjectRotation[i].eulerAngles.y));
            }
